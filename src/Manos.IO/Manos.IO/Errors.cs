...
namespace Manos.IO
{
    internal static class Errors
    {
        ...
        {
            return Stdlib.strerror(NativeConvert.ToErrno(errno));
        }

        public static SocketError? ErrorToSocketError(int errno)
        {
            Errno err = NativeConvert.ToErrno(errno);
            switch (err)
            {
        ...
        {
            SocketError? err = ErrorToSocketError(errno);
            if (err != null)
            {
                return new IOException(text, new SocketException(err.Value));
            }
            else
            {
        ...
        public static SocketException SocketFailure(string text, int errno)
        {
            SocketError? err = ErrorToSocketError(errno);
            return new SocketException(text, err ?? SocketError.Failure);
        }
    }
}