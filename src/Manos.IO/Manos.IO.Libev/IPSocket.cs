...
namespace Manos.IO.Libev
{
        ...
        where TStream : IStream<TFragment>
    {
        ...
        public override IPEndPoint LocalEndpoint
        {
            get
            {
                ...
                if (localname == null)
                {
                    int err;
                    ManosIPEndpoint ep;
                    int result = SocketFunctions.manos_socket_localname_ip(fd, out ep, out err);
                    if (result < 0)
                    {
        ...
        public override IPEndPoint RemoteEndpoint
        {
            get
            {
                ...
                if (peername == null)
                {
                    int err;
                    ManosIPEndpoint ep;
                    int result = SocketFunctions.manos_socket_peername_ip(fd, out ep, out err);
                    if (result < 0)
                    {
        ...
        public override void Bind(IPEndPoint endpoint)
        {
            ...
            int result = SocketFunctions.manos_socket_bind_ip(fd, ref ep, out err);
            if (result < 0)
            {
