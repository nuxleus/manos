...
namespace Manos.Spdy.Tests
{
    ...
    public class GeneratorTests
    {
        ...
        public void GenerateRstStream()
        {
            ...
            byte[] fromclass = frame.Serialize();
            Assert.AreEqual(16, fromclass.Length, "Lengths");
                //first 8 is rst_stream length always, 2nd 8 is for control frame header
            Assert.AreEqual(0x80, fromclass[0], "Control Bit");
            Assert.AreEqual(0x02, fromclass[1], "Version");
        ...
        {
            var frame = new VersionFrame();
            frame.Version = 1;
            frame.Flags = 0x00;
            frame.SupportedVersions = new[] {2, 3};
            byte[] fromclass = frame.Serialize();
            Assert.AreEqual(0x80, fromclass[0], "Control Bit");
